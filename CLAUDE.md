...
> NotionNext is an agent-driven project. Read this file first. Then follow the documentation map below before writing any code.
...
- [`docs/project/README.md`](docs/project/README.md) — project overview, goals, and tech stack.
...
- [`docs/frontend/fsd-overview.md`](docs/frontend/fsd-overview.md) — what FSD is and why it is used.
...
- **No redundant UI copy.** Do not repeat information already conveyed by a title, icon, selected state, or surrounding context.
...
- A feature owns a complete user scenario (e.g. "search", "comments"). It contains UI, model, API, and logic specific to that scenario.
- An entity owns domain data and rules (e.g. `Post`, `Comment`). It has no UI and no HTTP client logic.
- `shared` contains framework-agnostic utilities, UI primitives, and configuration usable by any layer.
...
Next.js Pages Router auto-detects `frontend/src/pages/` as the routing root, so it doubles as the FSD `pages` layer.
...
- **`domain/`** contains business rules. No dependencies on frameworks, databases, or transport details.
...
- **`interfaces/`** adapts the application layer to the outside world. In NotionNext the concrete HTTP entry points are the Next.js API route handlers in `frontend/src/pages/api/`.
...
- `scope`: module or layer name, e.g., `frontend`, `backend`, `auth`, `themes`.
- `subject`: lowercase, concise, no trailing period.

Example: `feat(backend): add notion sync use case`.
