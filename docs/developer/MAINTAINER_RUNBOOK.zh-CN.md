...
| 工具                                                               | 作用                                                     |
| ------------------------------------------------------------------ | -------------------------------------------------------- |
| [ci.yml](../../.github/workflows/ci.yml)                           | PR 与 `main`：lint、type-check、test、lockfile、文档构建 |
| [codeql-analysis.yml](../../.github/workflows/codeql-analysis.yml) | JavaScript / TypeScript 安全扫描                         |
| [dependabot.yml](../../.github/dependabot.yml)                     | 每周依赖 PR（默认跳过 major）                            |
| CODEOWNERS                                                         | 仅高影响路径 @Founder                                    |

生产 Cloudflare Worker 不由 GitHub Actions 发布。Cloudflare Workers Builds
监听 `main` push，并在 Cloudflare 的构建环境中完成构建与部署。
