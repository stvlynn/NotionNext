...
- Notion 公开页面 API 有时不会返回 `html_artifact` 标记；NotionNext 也会识别来源为 `attachment:` 且文件名以 `.html` 或 `.htm` 结尾的 Embed 块。
- NotionNext 请求公开页面数据时会发送项目 User-Agent，避免 Notion 拒绝匿名的 HTML artifact 刷新请求。
