type ThemeColorToken = Record<string, string>
...
const DEFAULT_LIGHT: ThemeColorToken = {
  PRIMARY: '#2563eb',
  BG: '#ffffff',
  CARD: '#ffffff',
  TEXT: '#111827',
...
const DEFAULT_DARK: ThemeColorToken = {
  PRIMARY: '#60a5fa',
  BG: '#000000',
  CARD: '#111827',
  TEXT: '#e5e7eb',
...
const THEME_COLOR_DEFAULTS: Record<string, ThemeColorToken> = {
...
const BASE_PALETTE: Array<[string, string]> = [
  ['PRIMARY', '主色'],
  ['BG', '页面背景'],
  ['CARD', '卡片背景'],
  ['TEXT', '主文字'],
...
export function getThemeColorDefault(themeId: string, token: string): string {
...
export function getThemeColorCssVar(themeId: string, token: string): string {
  return `--${themeId}-color-${token.toLowerCase().replace(/_/g, '-')}`
}

export function withBaseThemePalette(
  themeId: string,
