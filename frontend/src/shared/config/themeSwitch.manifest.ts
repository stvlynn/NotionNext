...
type ThemeSettingType = 'boolean' | 'text' | 'number' | 'select'
...
interface ThemeSwitchManifestRow {
  name?: string
  summary?: string
  cover?: string
  coverWebp?: string
...
export const THEME_SWITCH_MANIFEST: Record<string, ThemeSwitchManifestRow> = {
  endspace: {
    name: 'Endspace',
...
const THEME_CONFIGS: Record<string, Record<string, unknown>> = {
  claude: claudeConfig,
...
function inferThemeSettings(
  themeId: string,
  manualSettings: ThemeSetting[] = []
): ThemeSetting[] {
  ...
  return Object.entries(config)
    ...
    .map(([key, value]) => normalizeSetting({
      ...
      defaultValue: value as ThemeSettingValue
    }, themeId))
}

const SELECT_OPTIONS_BY_KEY: Record<string, ThemeSettingOption[]> = {
  NEXT_NAV_TYPE: [
...
const CONFIG_LABEL_WORDS: Record<string, string> = {
  ABOUT: '关于',
  AD: '广告',
  ADSENSE: 'Adsense',
  ANALYTICS: '统计',
...
const CONFIG_HELP_RULES: Array<[RegExp, string]> = [
...
function normalizeSetting(item: ThemeSetting, themeId: string): ThemeSetting {
...
function inferSelectOptions(item: ThemeSetting): ThemeSettingOption[] | null {
  const configuredOptions = SELECT_OPTIONS_BY_KEY[item.key]
...
function formatConfigHelp(key: string): string {
...
function formatConfigLabel(key: string, themeId: string): string {
 ...
 */
export function getThemeSwitchMeta(themeId: string): ThemeSwitchMeta {
