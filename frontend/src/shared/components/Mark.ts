...
let markJsLoadPromise: Promise<string> | null = null

function escapeSearchKeyword(search: any) {
 ...
 */
export default async function replaceSearchResult({ doms, search, target }: any) {
  ...
  try {
    ...
    const markContainer = (container: any) => {
