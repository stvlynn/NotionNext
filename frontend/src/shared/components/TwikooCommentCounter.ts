...
const TwikooCommentCounter = (props: any) => {
   ...
   * @param {*} posts
   */
  const fetchTwikooData = async (posts: any) => {
    ...
    try {
      ...
      twikoo.getCommentsCount({
        ...
        urls: posts?.map((post: any) => post.slug), // 不包含协议、域名、参数的文章路径列表，必传参数
  ...
  const updateCommentCount = () => {
    ...
    props.posts.forEach((post: any) => {
      const matchingRes = commentsData.find((r: any) => r.url === post.slug)
      if (matchingRes) {
        // 修改评论数量div
