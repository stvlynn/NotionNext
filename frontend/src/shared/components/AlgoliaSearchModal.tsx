 ...
 */
export default function AlgoliaSearchModal({ cRef }: any) {
  ...
  // 跳转Search结果
  const onJumpSearchResult = (indexOverride?: number) => {
    if (searchResults.length > 0) {
      const searchResult = searchResults[indexOverride ?? activeIndex]
  ...
  const searchIndex = client.initIndex(siteConfig('ALGOLIA_INDEX'))
  ...
  const handleSearch = async (query: any, page: any) => {
    ...
    setIsLoading(true)
    try {
      const res = await searchIndex.search(query, { page, hitsPerPage: 10 })
        ...
        <ul className='flex-1 overflow-auto'>
          {searchResults.map((result: any, resultIndex: number) => (
            <li
              ...
              rounded-lg
              ${activeIndex === resultIndex ? 'bg-blue-600 dark:bg-yellow-600' : ''}`}>
              <a
                className={`${activeIndex === resultIndex ? ' text-white' : ' text-black dark:text-gray-300 '}`}>
                {result.title}
              </a>
            </li>
          ))}
        ...
        <div className='flex items-center justify-between mt-2 sm:text-sm text-xs dark:text-gray-300'>
          {totalHit === 0 && (
            <div className='flex items-center'>
              {ShortCutActions.map((action: any, index: any) => {
                return (
                  <Fragment key={index}>
...
function Pagination(props: any) {
  ...
  return (
    <div className='flex space-x-1 w-full justify-center py-1'>
      {Array.from({ length: totalPage }, (_: any, i: any) => {
        const classNames =
          page === i
