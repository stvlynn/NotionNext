...
function getNodesWithAdsByGoogleClass(node: any) {
  const adsNodes: any[] = []

  // 检查节点及其子节点是否包含 adsbygoogle 类
  function checkNodeForAds(node: any) {
 ...
 * @returns
 */
export const initGoogleAdsense = (ADSENSE_GOOGLE_ID: any) => {
...
const AdEmbed = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      // 遍历找到的元素
      notionTextElements?.forEach((element: any) => {
        // 检查元素的内容是否为 '<ins/>'
