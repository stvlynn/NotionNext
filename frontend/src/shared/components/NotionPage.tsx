 ...
 * @returns
 */
const NotionPage = ({ post, className }: any) => {
  // 是否关闭数据库和画册的点击跳转
  ...
  useEffect(() => {
    // Spoiler文本功能
    if (SPOILER_TEXT_TAG) {
      import('lodash/escapeRegExp').then((escapeRegExp: any) => {
        Promise.all([
...
const hasCodeBlock = (blockMap: any) => {
  const blocks = blockMap?.block
  if (!blocks) return false
  return Object.values(blocks).some(
    (item: any) => item?.value?.type === 'code'
  )
}

const NotionEmbed = ({ block }: any) => {
 ...
 */
const processGalleryImg = (zoom: any) => {
  setTimeout(() => {
    if (isBrowser) {
...
const Code = dynamic(
  () =>
    import('react-notion-x/build/third-party/code').then((m: any) => {
      return m.Code
    }),
  { ssr: false }
)
...
const Equation = dynamic(
  () =>
    import('@/components/Equation').then(async (m: any) => {
      // 化学方程式
...
//   }
// )
const Pdf = dynamic(() => import('@/components/Pdf').then((m: any) => m.Pdf), {
  ssr: false
})
...
const TweetEmbed = dynamic(
  () => import('react-tweet-embed').then((m: any) => m.default || m),
  {
    ssr: false
  }
)
...
const AdEmbed = dynamic(
  () => import('@/components/GoogleAdsense').then((m: any) => m.AdEmbed),
  { ssr: true }
)
...
const Modal = dynamic(
  () => import('react-notion-x/build/third-party/modal').then((m: any) => m.Modal),
  { ssr: false }
)

const Tweet = ({ id }: any) => {
  return <TweetEmbed {...({ tweetId: id } as any)} />
}
...
const NotionQuote = ({ block, children }: any) => {
...
const NotionText = ({ value }: any) => {
  if (!Array.isArray(value)) return null
  return value.map((segment: any, i: any) => {
