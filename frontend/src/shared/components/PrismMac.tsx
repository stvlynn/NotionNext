...
const getNotionArticle = () => {
  ...
  return candidates.sort((a: any, b: any) => score(b) - score(a))[0] || null
}
...
const loadPrismThemeCSS = (
  isDarkMode: any,
  prismThemeSwitch: any,
  prismThemeDarkPath: any,
  prismThemeLightPath: any,
  prismThemePrefixPath: any
) => {
  let PRISM_THEME
  let PRISM_PREVIOUS
  if (prismThemeSwitch) {
 ...
 * 将代码块转为可折叠对象
 */
const renderCollapseCode = (codeCollapse: any, codeCollapseExpandDefault: any) => {
  if (!codeCollapse) {
    return
  }
  ...
  for (const codeBlock of codeBlocks) {
    try {
      ...
      function setExpanded(expanded: any) {
 ...
 * 将mermaid语言 渲染成图片
 */
const renderMermaid = (mermaidCDN: any) => {
...
function renderPrismMac(codeLineNumbers: any, codeMacBar: any) {
  ...
  if (codeLineNumbers) {
    ...
    if (codeBlocks) {
      Array.from(codeBlocks).forEach((item: any) => {
