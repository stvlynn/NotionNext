...
function ThemeTierBadge ({ tier, labels }: any) {
...
const EMPTY_CONSOLE_ITEMS: any[] = []

function PaletteField ({ item, value, copyValue, isHexColor, updateItem, resetItem, copyText }: any) {
  return (
    <div
      ...
      <div className='mb-2 flex items-start gap-2'>
        <input
          ...
          onChange={(event: any) => updateItem(item, event.target.value)}
...
function ThemeConsole ({ meta, onClose }: any) {
  ...
  const readSettingValue = (item: any) => siteConfig(item.key, item.defaultValue, THEME_CONFIG || {})

  const hexToHue = (value: any) => {
  ...
  const getPreviewValue = (item: any, value: any) => {
    if (item.key === 'FUWARI_THEME_COLOR_HUE') return hueToHex(value)
    return value
  }

  const getConsoleAlias = useCallback((item: any) => {
  ...
  const writePreviewColor = useCallback((root: any, item: any, value: any) => {
  ...
  useEffect(() => {
    ...
    declaredPalette.forEach((item: any) => {
      nextValues[item.key] =
  ...
  const isHexColor = (value: any) => /^#[0-9a-f]{6}$/i.test(String(value).trim())
  const isDarkPaletteItem = (item: any) =>
    /_DARK($|_)/.test(item.key) || String(item.label || '').includes('深色')
  const lightPalette = palette.filter((item: any) => !isDarkPaletteItem(item))
  ...
  const updateItem = (item: any, value: any) => {
    ...
    setValues((prev: any) => ({ ...prev, [item.key]: previewValue }))
    const root = getRoot()
  ...
  const resetItem = (item: any) => updateItem(item, item.defaultValue)

  const updateSetting = (item: any, value: any) => {
    setSettingValues((prev: any) => ({ ...prev, [item.key]: value }))
    updateRuntimeConfigOverride?.(item.key, value)
  }

  const updateSelectSetting = (item: any, selectedValue: any) => {
    const option = (item.options || []).find((option: any) => String(option.value) === selectedValue)
    updateSetting(item, option ? option.value : selectedValue)
  }

  const resetSetting = (item: any) => updateSetting(item, item.defaultValue)
  ...
  const showNotice = (text: any) => {
    setNotice(text)
    window.clearTimeout(noticeTimerRef.current)
    noticeTimerRef.current = window.setTimeout(() => setNotice(''), 1800)
  }

  const copyText = async (text: any, message: any = '配置已复制到剪贴板') => {
    try {
  ...
  const copyAll = () => {
    const settingText = settings.map((item: any) => {
  ...
  const parseConfigText = (text: any) => {
          ...
          <section className='overflow-hidden rounded-2xl border border-gray-100 bg-gray-50/70 dark:border-gray-800 dark:bg-gray-900/60'>
            ...
            {openSections.settings && (
              <div className='grid grid-cols-1 gap-2 border-t border-gray-100 p-3 dark:border-gray-800 sm:grid-cols-2'>
                {settings.length ? settings.map((item: any) => {
                  const value = settingValues[item.key] ?? readSettingValue(item)
                  return (
                    <div
                      key={item.key}
                      ...
                      {item.type === 'boolean' ? (
                        ...
                        <select
                          value={String(value)}
                          onChange={(event: any) => updateSelectSetting(item, event.target.value)}
                        ...
                        <input
                          ...
                          value={value}
                          onChange={(event: any) => updateSetting(item, item.type === 'number' ? Number(event.target.value) : event.target.value)}
        ...
        <div
          ...
          <div className='flex min-w-0 items-center gap-0.5'>
            ...
            <button
              ...
              onClick={() => {
                setConsoleVisible((visible: any) => !visible)
              }}
              title='配置主题'
              aria-label='配置主题'
