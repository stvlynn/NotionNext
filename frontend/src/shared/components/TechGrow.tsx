...
const getFirstConfig = (keys: any, defaultVal: any = null) => {
...
const getReadmoreWrapperIn = (root: any) =>
  root?.querySelector?.('#readmore-wrapper, #read-more-wrap') || null

const normalizePreviewHeight = (height: any) => {
...
const isReadmoreRelatedUrl = (url: any) =>
  typeof url === 'string' &&
  /(techgrow|readmore|open\.techgrow\.cn|qiniu\.techgrow\.cn)/i.test(url)

const installReadmoreNetworkProbe = (debug: any) => {
...
const installReadmoreAlertProbe = (debug: any) => {
  if (!debug || typeof window === 'undefined') {
    return
  }
...
const logReadmoreState = (debug: any, contentId: any, stage: any) => {
  ...
  const loadReadmore = async () => {
    try {
      ...
      if (ReadmorePlugin) {
        ...
        ;[0, 150, 400, 900].forEach((delay: any) => {
          setTimeout(() => {
...
// 定义禁用和恢复目录项点击的函数
const toggleTocItems = (disable: any) => {
 ...
 */
function isPathInList(path: any, listStr: any) {
  if (!path || !listStr) {
    return false
  }
