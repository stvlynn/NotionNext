...
const ShareButtons = ({ post }: any) => {
  ...
  const hashTags = tags.map((tag: any) => `#${tag}`).join(',')
  const body =
  ...
  const openRedirectShare = (base: any) => {
  ...
  const buildShareUrl = (service: any) => {
  ...
  const renderCommonShareButton = (service: any) => {
    const shareLink = buildShareUrl(service)
    if (!shareLink) return null
    const iconClass = (SHARE_ICON_CLASS as Record<string, string>)[service] || 'fas fa-share-alt'
    const bgClass = (SHARE_BG_CLASS as Record<string, string>)[service] || 'bg-gray-600'
    return (
      <button
        aria-label={service}
        key={service}
  ...
  return (
    <>
      {services.map((singleService: any) => {
