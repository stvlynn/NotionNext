...
const ExternalPlugin = (props: any) => {
  ...
  const externalCssList = useMemo(() => {
    return Array.isArray(CUSTOM_EXTERNAL_CSS)
      ? CUSTOM_EXTERNAL_CSS.filter((url: any) => !!url)
      : []
  }, [CUSTOM_EXTERNAL_CSS])

  const externalJsList = useMemo(() => {
    return Array.isArray(CUSTOM_EXTERNAL_JS)
      ? CUSTOM_EXTERNAL_JS.filter((url: any) => !!url)
      : []
  }, [CUSTOM_EXTERNAL_JS])
  ...
  useEffect(() => {
    ...
    const scheduleTask = (callback: any) => {
    ...
    return () => {
      cancelTasks.forEach((cancel: any) => cancel?.())
    }
...
const Analytics = dynamic(
  () =>
    import('@vercel/analytics/react').then((m: any) => {
      return m.Analytics
    }),
  { ssr: false }
)
...
const LoadingProgress = dynamic(
  () => import('@/components/LoadingProgress').then((m: any) => m.default),
  {
    ssr: false
  }
)
