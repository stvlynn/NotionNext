 ...
 * @constructor
 */
const WebmentionCount = ({ target }: any) => {
  const initialCounts = {
    count: 0,
    type: {
      like: 0,
  ...
  const fetchCounts = async (target: any) => {
...
const WebmentionReplies = ({ target }: any) => {
  const [mentions, setMentions] = useState<any[]>([])
  const fetchMentions = async (target: any) =>
    fetch(
  ...
  const distinctMentions = [
    ...new Map(mentions.map((item: any) => [item.author.url, item])).values()
  ].sort(
    (a: any, b: any) =>
  ...
  const replies = mentions.filter(
    (mention: any) => 'in-reply-to' in mention && 'content' in mention
  )
  ...
  return (
    <div>
      ...
      <div className='webmention-avatars'>
        {distinctMentions.map((reply: any) => (
...
const WebMentionBlock = ({ frontMatter }: any) => {
