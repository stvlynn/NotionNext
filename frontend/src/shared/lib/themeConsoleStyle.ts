...
type ThemeConsoleConfig = Record<string, unknown>

interface ThemeConsoleStyleOptions {
  rootId?: string
}

const TOKENS = ['PRIMARY', 'BG', 'CARD', 'TEXT', 'TEXT_SECONDARY', 'BORDER'] as const

function readColor(themeId: string, token: string, config?: ThemeConsoleConfig): string {
  const key = `${String(themeId).toUpperCase()}_COLOR_${token}`
  return siteConfig<string>(key, getThemeColorDefault(themeId, token), config)
}

export function themeConsoleStyle(
  themeId: string,
  config?: ThemeConsoleConfig,
  options: ThemeConsoleStyleOptions = {}
): string {
