...
export const sha256Digest = (str: unknown): string => sha256(String(str))

/** Notion 密码字段直接填入的预计算 SHA256（64 位 hex） */
export const isSHA256Digest = (str: unknown): boolean =>
  typeof str === 'string' && /^[a-fA-F0-9]{64}$/.test(str.trim())

/** 旧版 md5(slug+明文) 的 32 位 hex，同步回字段时原样保留 */
export const isMd5Digest = (str: unknown): boolean =>
 ...
 */
export const getPasswordStoragePath = (path: string): string => {
  if (!path) {
    return '/'
  }
  try {
 ...
 */
export const getPasswordQuery = (path: string): string[] => {
  // 使用 URL 对象解析 URL
