...
type SitemapConfig = Record<string, unknown>
...
type PendingSitemapUrl = Omit<SitemapUrl, 'loc'> & { loc: string | null }
...
export function generateSitemapXml({
  ...
  ] satisfies PendingSitemapUrl[]).filter(
    (item): item is SitemapUrl => Boolean(item?.loc)
  )
  // 循环页面生成
