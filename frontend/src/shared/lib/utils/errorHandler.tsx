import React, { type ComponentType, type ErrorInfo as ReactErrorInfo, type ReactNode } from 'react'
/**
 * 统一错误处理工具类
 */
...
export class AppError extends Error {
  ...
  constructor(
    message: string,
    type: ErrorType = ErrorTypes.UNKNOWN_ERROR,
    statusCode = 500,
...
export class NetworkError extends AppError {
  constructor(message = '网络连接失败', details: ErrorDetails | null = null) {
...
export class ValidationError extends AppError {
  constructor(message = '数据验证失败', details: ErrorDetails | null = null) {
...
export class ErrorHandler {
  static logError(error: Error | AppError, context = ''): void {
    const appError = error as Error & Partial<AppError>
    const errorInfo: ErrorReport = {
      message: error.message,
      type: appError.type || ErrorTypes.UNKNOWN_ERROR,
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      // 生产环境下只输出基本信息
      console.error('Error:', error.message, 'Type:', appError.type)
    }
  ...
  static reportError(errorInfo: ErrorReport): void {
    // 错误上报逻辑
  ...
  static handleApiError(error: ErrorResponseLike): never {
    if (error.response) {
      // 服务器响应了错误状态码
  ...
  static async safeExecute<T>(
    ...
    try {
      ...
      this.logError(error instanceof Error ? error : new AppError(String(error)), context)
      return fallback
    }
  }

  static createErrorBoundary(fallbackComponent?: ReactNode): ComponentType<{
    children?: ReactNode
  }> {
