 ...
 */
export function generateSitemap(allPosts: SitemapPost[] = []): string {
  const LINK = siteConfig<string>('LINK')
  ...
  const postPages = allPosts
    ...
    .map(post => ({
      url: `${LINK}/${post.slug}`,
      lastmod: new Date(post.lastEditedTime || post.date || 0).toISOString(),
      changefreq: 'weekly',
      priority: '0.9'
    }))
 ...
 */
export function generateRSS(allPosts: SitemapPost[] = []): string {
  const LINK = siteConfig<string>('LINK')
  const TITLE = siteConfig<string>('TITLE')
  const DESCRIPTION = siteConfig<string>('DESCRIPTION')
  const AUTHOR = siteConfig<string>('AUTHOR')
  const LANG = siteConfig<string>('LANG')
...
${publishedPosts.map(post => `    <item>
      ...
      <pubDate>${new Date(post.date || 0).toUTCString()}</pubDate>
...
export function generateRobotsTxt(): string {
  const LINK = siteConfig<string>('LINK')
  const ROBOTS_ALLOW = siteConfig<boolean>('ROBOTS_ALLOW', true)
...
export function generateManifest(): Record<string, unknown> {
  const TITLE = siteConfig<string>('TITLE')
  const DESCRIPTION = siteConfig<string>('DESCRIPTION')
  const THEME_COLOR = siteConfig<string>('THEME_COLOR', '#000000')
  const BACKGROUND_COLOR = siteConfig<string>('BACKGROUND_COLOR', '#ffffff')
