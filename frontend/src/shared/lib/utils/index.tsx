 ...
 * 组件持久化
 */
export const memorize = <P extends object>(Component: ComponentType<P>) => {
 ...
 * @returns
 */
export function sliceUrlFromHttp(str: unknown): string {
 ...
 * @param {*} str
 */
export function convertUrlStartWithOneSlash(str: unknown): string {
  if (!str) {
    return '#'
  }
 ...
 * @returns {boolean}
 */
export function isUrlLikePath(str: unknown): boolean {
 ...
 * @returns {boolean}
 */
export function isHttpLink(str: unknown): boolean {
 ...
 * @returns {boolean}
 */
export function isMailOrTelLink(href: unknown): boolean {
  if (typeof href !== 'string') return false
  return /^(mailto:|tel:)/i.test(href)
}

// 检查一个字符串是否UUID https://ihateregex.io/expr/uuid/
export function checkStrIsUuid(str: string): boolean {
  if (!str) {
    return false
  }
  // 使用正则表达式进行匹配
...
export function checkStrIsNotionId(str: string): boolean {
  if (!str) {
    return false
  }
  // 使用正则表达式进行匹配
...
export function loadExternalResource(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    let tag: HTMLLinkElement | HTMLScriptElement | undefined
 ...
 * @param sources
 */
export function mergeDeep<T extends PlainRecord>(
  target: T,
  ...sources: Array<PlainRecord | undefined>
): T {
  ...
  if (isObject(target) && isObject(source)) {
    for (const key in source) {
      if (isObject(source[key])) {
        ...
        mergeDeep(target[key] as PlainRecord, source[key] as PlainRecord)
      } else {
 ...
 * @returns {boolean}
 */
export function isObject(item: unknown): item is PlainRecord {
    ...
    for (const key in source) {
      if (Object.prototype.hasOwnProperty.call(obj, key)) {
        if (source[key] instanceof Date) {
          newObj[key] = new Date(source[key].getTime()).toISOString()
        } else {
 ...
 * @returns {*}
 */
export const getListByPage = function <T>(
  list: T[],
  pageIndex: number,
  pageSize: number
): T[] {
 ...
 * @param {*} node
 */
export const scanAndConvertToLinks = (node: Node): void => {
 ...
 * @returns
 */
export function getLastSegmentFromUrl(url: unknown): string {
  if (!url) {
    return ''
  }
