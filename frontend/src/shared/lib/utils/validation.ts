...
export class Validator {
   ...
   * @returns {boolean}
   */
  static isValidSlug(slug: unknown): boolean {
   ...
   * @returns {boolean}
   */
  static isValidNotionId(id: unknown): boolean {
   ...
   * @returns {boolean}
   */
  static isValidIpAddress(ip: unknown): boolean {
   ...
   * @returns {boolean}
   */
  static isValidUsername(username: unknown): boolean {
   ...
   * @returns {boolean}
   */
  static isValidPassword(password: unknown): boolean {
   ...
   * @returns {boolean}
   */
  static isValidLength(str: unknown, min = 0, max = Infinity): boolean {
   ...
   * @returns {boolean}
   */
  static isValidNumber(num: unknown, min = -Infinity, max = Infinity): boolean {
   ...
   * @returns {boolean}
   */
  static isValidArray(arr: unknown, minLength = 0, maxLength = Infinity): boolean {
...
export class Sanitizer {
   ...
   * @returns {string}
   */
  static stripHtml(str: unknown): string {
   ...
   * @returns {string}
   */
  static sanitizeXss(str: unknown): string {
    if (!str || typeof str !== 'string') return ''
   ...
   * @returns {string}
   */
  static sanitizeSql(str: unknown): string {
    if (!str || typeof str !== 'string') return ''
   ...
   * @returns {object|null}
   */
  static sanitizeJson(jsonStr: unknown): SanitizedValue | null {
   ...
   * @returns {any}
   */
  static deepSanitizeObject(obj: unknown): SanitizedValue {
...
export class RateLimiter {
   ...
   * @returns {boolean}
   */
  isRateLimited(identifier: string, limit = 100, windowMs = 60000): boolean {
    const now = Date.now()
  ...
  cleanup(): void {
    ...
    for (const [identifier, requests] of Array.from(this.requests.entries())) {
      const validRequests = requests.filter((timestamp: number) => timestamp > now - 3600000) // 1小时
