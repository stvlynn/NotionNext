...
// 扫描项目 /themes下的目录名
const themes = scanSubdirectories(path.resolve(__dirname, 'src/shared/themes'))
// 检测用户开启的多语言
...
const nextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    // FSD/DDD path aliases (mirror frontend/jsconfig.json so server & client bundles resolve identically)
    const src = path.resolve(__dirname, 'src')
    const backend = path.resolve(__dirname, '..', 'backend', 'src')
    config.resolve.alias = {
      ...config.resolve.alias,
      '@/components': path.resolve(src, 'shared/components'),
      '@/lib/db': path.resolve(backend, 'infrastructure/db'),
      '@/lib/cache': path.resolve(backend, 'infrastructure/cache'),
      '@/lib/server': path.resolve(backend, 'infrastructure/claude'),
      '@/lib/middleware': path.resolve(backend, 'infrastructure/middleware'),
      '@/lib/build': path.resolve(backend, 'application/build'),
      '@/lib/site': path.resolve(backend, 'application/site'),
      '@/lib': path.resolve(src, 'shared/lib'),
      '@/themes': path.resolve(src, 'shared/themes'),
  ...
  ,
  experimental: {
    // Allow SWC to compile TypeScript under ../backend (imported via @/lib/* aliases).
    // Without this, webpack parses backend .ts as plain JS and fails on `interface`.
    externalDir: true,
    // cpus: 1,
